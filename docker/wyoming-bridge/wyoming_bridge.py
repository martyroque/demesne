...
class WyomingBridge:
    ...
    async def transcribe_audio(self, audio_data: bytes) -> tuple[Optional[str], Optional[str]]:
        """
...
async def on_startup(app):
    ...
    logger.info(f"Wyoming bridge started, connecting to {config.whisper_host}:{config.whisper_port}")
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting Wyoming HTTP Bridge on {config.bridge_host}:{config.bridge_port}")
    web.run_app(
        app,
